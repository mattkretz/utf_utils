...
//
ptrdiff_t
Convert16_KewbSimdSmTab(string const& src, size_t reps, u16string& dst)
{
    char8_t const*  pSrcBuf = (char8_t const*) &src[0]; //- Pointer to source buffer
    char8_t const*  pSrcEnd = pSrcBuf + src.size();     //- Pointer to end of source buffer
    char16_t*       pDstBuf = &dst[0];                  //- Pointer to destination buffer
...
//
ptrdiff_t
Convert16_KewbSimdBgTab(string const& src, size_t reps, u16string& dst)
{
    char8_t const*  pSrcBuf = (char8_t const*) &src[0]; //- Pointer to source buffer
    char8_t const*  pSrcEnd = pSrcBuf + src.size();     //- Pointer to end of source buffer
    char16_t*       pDstBuf = &dst[0];                  //- Pointer to destination buffer
...
TestAllConversions16(string const& fname, bool isFile, size_t repShift, bool tblCmp)
{
    ...
    if (tblCmp)
    {
        ...
        tdiff = TestOneConversion16(&Convert16_KewbSimdSmTab, u8src, reps, u16answer, "kewb-vir-simd-small-table");
        times.push_back(tdiff);
        algos.emplace_back("kewb-vir-simd-small-table");

        tdiff = TestOneConversion16(&Convert16_KewbSimdBgTab, u8src, reps, u16answer, "kewb-vir-simd-big-table");
        times.push_back(tdiff);
    ...
    else
    {
        ...
        //add("vir1", [](auto a, auto b, auto c) { return Convert16_KewbApi(a, b, c, UtfUtils::VirConvert<1>); });
        add("vir2", [](auto a, auto b, auto c) { return Convert16_KewbApi(a, b, c, UtfUtils::VirConvert<2>); });
        add("vir3", [](auto a, auto b, auto c) { return Convert16_KewbApi(a, b, c, UtfUtils::VirConvert<3>); });
        add("vir4", [](auto a, auto b, auto c) { return Convert16_KewbApi(a, b, c, UtfUtils::VirConvert<4>); });
        add("vir2p", [](auto a, auto b, auto c) { return Convert16_KewbApi(a, b, c, UtfUtils::VirConvert<2, true>); });
        add("vir3p", [](auto a, auto b, auto c) { return Convert16_KewbApi(a, b, c, UtfUtils::VirConvert<3, true>); });
        add("vir4p", [](auto a, auto b, auto c) { return Convert16_KewbApi(a, b, c, UtfUtils::VirConvert<4, true>); });
    }
...
TestFiles16(string const& dataDir, size_t repShift, file_list const& files, bool tblCmp)
{
    ...
    std::ofstream datafile("utf-16.dat", std::ios::out);
    datafile << "file";
